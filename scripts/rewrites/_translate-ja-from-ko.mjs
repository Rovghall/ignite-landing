#!/usr/bin/env node
/** Translate KO paragraph keys to JA and build fix modules + patch batch01Ja */
import fs from 'fs';
...
const hJa = {
  ...
  '무료 등급이 지방 감량을 깨는 지점': '無料プランが脂肪減を壊す所',
  ...
  '14일 무료 등급 스트레스 테스트': '14日無料プラン ストレステスト',
...
function buildJaPost(slug, sections) {
  const [title, description] = titles[slug];
  return { slug, title, date: koAll[slug].date, description, sections };
}
...
async function main() {
  ...
  for (const slug of slugs) {
    ...
    for (const s of ko.sections) {
      const sec = { body: [] };
      if (s.heading) sec.heading = hJa[s.heading] || s.heading.replace('결론', 'まとめ');
      for (const p of s.body) {
        sec.body.push(await translateKoJa(p));
        await new Promise((r) => setTimeout(r, 120));
      }
      sections.push(sec);
    }
    jaPosts[slug] = buildJaPost(slug, sections);
