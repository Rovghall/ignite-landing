/**
 * Merge batch04 native translations into content/<locale>/blog.json (EN order).
 * Usage: node scripts/rewrites/merge-batch04.mjs
 */
...
const LOCALE_FILES = {
  pt: '_batch04-pt-data.json',
  'pt-br': '_batch04-pt-br-data.json',
  es: '_batch04-es-data.json',
  fr: '_batch04-fr-data.json',
  de: '_batch04-de-data.json',
  it: '_batch04-it-data.json',
  nl: '_batch04-nl-data.json',
  no: '_batch04-no-data.json',
  sv: '_batch04-sv-data.json',
  ja: '_batch04-ja-data.json',
  ko: '_batch04-ko-data.json',
  zh: '_batch04-zh-data.json',
}

const slugs = JSON.parse(fs.readFileSync(path.join(REWRITES, 'batch04-slugs.json'), 'utf8'))
...
for (const [locale, filename] of Object.entries(LOCALE_FILES)) {
  ...
  console.log(`[${locale}] +${added} ~${updated} → ${merged.length} posts (batch04 ${have}/20)`)
}
