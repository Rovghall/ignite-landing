#!/usr/bin/env node
/** Generate _fix-ja-1-10-native.mjs and _fix-ja-11-20-native.mjs from KO + native JA section text */
import fs from 'fs';
...
// Import ZH as semantic source and map to JA via fixesZh modules
...
// zh->ja paragraph map: use KO headings + zh body translated in _ja-zh-map.json if exists
const mapPath = path.join(__dirname, '_ja-from-zh-map.json');
if (!fs.existsSync(mapPath)) {
  console.error('Create', mapPath, 'with slug->sections native JA bodies');
  process.exit(1);
}
const jaBodies = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
...
function build(slug) {
  ...
  if (!secs) throw new Error('missing ' + slug);
...
const ja1 = Object.fromEntries(slugs.slice(0, 10).map((s) => [s, build(s)]));
const ja11 = Object.fromEntries(slugs.slice(10).map((s) => [s, build(s)]));

fs.writeFileSync(path.join(__dirname, '_fix-ja-1-10-native.mjs'), '/** Native JA 1-10 */\n' + emit(ja1, 'jaPosts1_10') + '\n');
fs.writeFileSync(path.join(__dirname, '_fix-ja-11-20-native.mjs'), '/** Native JA 11-20 */\n' + emit(ja11, 'jaPosts11_20') + '\n');
console.log('OK', Object.keys(ja1).length, Object.keys(ja11).length);
