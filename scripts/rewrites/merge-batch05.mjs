/**
 * Merge batch05 native translations into content/<locale>/blog.json (EN order).
 * Usage: node scripts/rewrites/merge-batch05.mjs
 */
...
const LOCALE_FILES = {
  pt: '_batch05-pt-data.json',
  'pt-br': '_batch05-pt-br-data.json',
  es: '_batch05-es-data.json',
  fr: '_batch05-fr-data.json',
  de: '_batch05-de-data.json',
  it: '_batch05-it-data.json',
  nl: '_batch05-nl-data.json',
  no: '_batch05-no-data.json',
  sv: '_batch05-sv-data.json',
  ja: '_batch05-ja-data.json',
  ko: '_batch05-ko-data.json',
  zh: '_batch05-zh-data.json',
}

const slugs = JSON.parse(fs.readFileSync(path.join(REWRITES, 'batch05-slugs.json'), 'utf8'))
...
for (const [locale, filename] of Object.entries(LOCALE_FILES)) {
  ...
  console.log(`[${locale}] +${added} ~${updated} → ${merged.length} posts (batch05 ${have}/20)`)
}
