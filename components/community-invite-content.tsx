...
import {
  buildIgniteAiFriendDeepLink,
  communityInviteDisplayLabel,
  type CommunityInviteTarget,
} from '@/lib/community-invite'
import { cn } from '@/lib/utils'

type Props = {
  target: CommunityInviteTarget
}

export function CommunityInviteContent({ target }: Props) {
  const deepLink = useMemo(() => buildIgniteAiFriendDeepLink(target), [target])
  const label = communityInviteDisplayLabel(target)
  const [triedOpen, setTriedOpen] = useState(false)

  useEffect(() => {
    if (!deepLink || triedOpen) return
    setTriedOpen(true)
    // Best-effort open: if the app is installed, the OS may hand off.
    // Keep the landing visible as fallback when it does not.
    ...
    <main className="relative flex min-h-dvh flex-col overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-lg flex-1 flex-col px-5 pb-10 pt-8 sm:px-6">
        ...
        <div className="flex flex-1 flex-col items-center justify-center py-12 text-center">
          ...
          >
            <UserRound className="h-7 w-7" strokeWidth={2.2} aria-hidden />
          </div>
          ...
          {target.valid ? (
              ...
              <p className="mt-4 max-w-sm text-base leading-relaxed text-muted-foreground text-pretty sm:text-lg">
                {target.kind === 'handle' ? (
                  <>
                    ...
                    IGNITE AI app to send a friend request.
                  </>
                ) : (
                  <>
            ...
            <>
              ...
              <p className="mt-4 max-w-sm text-base leading-relaxed text-muted-foreground text-pretty sm:text-lg">
                This friend invite link is invalid or incomplete. Ask your friend to share
